...
async function verifySlackSignature(request, rawBody, signingSecret) {
  ...
  if (computed.length !== slackSig.length)
    return false;
  let diff = 0;
...
async function postMessage(channelId, text, botToken) {
  return slackCall("chat.postMessage", botToken, { channel: channelId, text });
}
__name(postMessage, "postMessage");
async function postReply(channelId, threadTs, text, botToken) {
...
__name(postReply, "postReply");
async function editMessage(channelId, messageTs, text, botToken) {
...
__name(slackCall, "slackCall");

// src/pco.js
var PCO_BASE = "https://api.planningcenteronline.com/services/v2";
async function fetchNextSundayPlan(appId, secret) {
...
__name(fetchNextSundayPlan, "fetchNextSundayPlan");
async function applyChangesToPco(planId, proposedChanges, appId, secret) {
  const results = [];
...
__name(applyChangesToPco, "applyChangesToPco");
...
function describeChange(change) {
  switch (change.type) {
    case "notice_title":
      return `Title updated for item ${change.item_id} \u2192 "${change.new_title}"`;
    case "remove_empty_notice":
      return `Deleted empty notice item ${change.item_id}`;
    case "remove_empty_highlight":
      return `Deleted empty highlight item ${change.item_id}`;
    case "fill_placeholder":
      return `Filled placeholder in item ${change.item_id} \u2192 "${change.volunteer_name}"`;
    default:
...
async function findNorthCampusServiceTypeId(appId, secret) {
  ...
  const nc = data.find(
    (st) => st.attributes.name?.toLowerCase().includes("north campus")
  );
  if (!nc) {
...
__name(findNorthCampusServiceTypeId, "findNorthCampusServiceTypeId");
...
__name(getNextPlan, "getNextPlan");
...
__name(getPlanTeams, "getPlanTeams");
function buildRosterIssues(teams) {
  ...
  for (const team of teams) {
    for (const member of team.members) {
      if (!watchedRoles.includes(member.role))
        continue;
    ...
    for (const role of watchedRoles) {
      ...
      if (!hasAny) {
        if (role === "Worship Leader" || role === "Sound Tech" || role === "Preacher") {
...
__name(buildRosterIssues, "buildRosterIssues");
...
__name(BetaModelInfosPage, "BetaModelInfosPage");
...
__name(BetaMessageBatchesPage, "BetaMessageBatchesPage");
...
__name(MessageBatchesPage, "MessageBatchesPage");
...
__name(ModelInfosPage, "ModelInfosPage");
...
async function classifyMessage(messageText, apiKey) {
...
__name(classifyMessage, "classifyMessage");
async function analyzePlan(slackMessage, pcoPlan, apiKey) {
  const client = new sdk_default({ apiKey });
...
async function refinePlan(feedbackText, storedState, pcoPlan, apiKey) {
  const client = new sdk_default({ apiKey });
...
__name(buildAnalysisSystemPrompt, "buildAnalysisSystemPrompt");
function buildAnalysisUserPrompt(slackMessage, pcoPlan) {
  const itemsText = pcoPlan.items.map((item) => `  ID: ${item.id}
  Title: ${item.attributes?.title ?? "(none)"}
  Description: ${item.attributes?.description ?? "(none)"}
  Type: ${item.attributes?.item_type ?? "unknown"}`).join("\n\n");
  const teamsText = pcoPlan.teams.map((team) => {
    const members = team.members.map((m) => `    - ${m.name} (${m.role}) \u2014 ${m.status}`).join("\n");
...
__name(buildAnalysisUserPrompt, "buildAnalysisUserPrompt");
...
function buildRefinementUserPrompt(feedbackText, storedState, pcoPlan) {
  ...
  const itemsText = pcoPlan.items.map((item) => `  ID: ${item.id} | Title: ${item.attributes?.title ?? "(none)"} | Desc: ${item.attributes?.description ?? "(none)"}`).join("\n");
...
__name(buildRefinementUserPrompt, "buildRefinementUserPrompt");
...
__name(nextSundayDate, "nextSundayDate");
function weekKey(sundayDate) {
...
async function handleNewChannelMessage(event, env) {
  ...
  const sunday = nextSundayDate();
  const pcoPlan = await fetchNextSundayPlan(env.PCO_APP_ID, env.PCO_SECRET);
...
__name(handleNewChannelMessage, "handleNewChannelMessage");
...
async function handleReaction(event, env) {
  ...
  if (stored.applied)
    return;
  if (event.item?.ts !== stored.bot_reply_ts)
    return;
  ...
  const summary = results.map((r) => r.ok ? `- Applied: ${r.description}` : `- FAILED: ${r.description} (${r.error})`).join("\n");
  await postReply(
    env.SLACK_CHANNEL_ID,
...
__name(formatAnalysisReply, "formatAnalysisReply");
