 ...
 * }
 */
export async function analyzePlan(slackMessage, pcoPlan, campusName, apiKey) {
  const client = new Anthropic({ apiKey });

  const systemPrompt = buildAnalysisSystemPrompt();
  const userPrompt = buildAnalysisUserPrompt(slackMessage, pcoPlan, campusName);
...
5. Bible reading placeholder: If an item's description contains a [Bible Reader Name] placeholder,
   ...
   "Bible Reader" role. Leave the rest of the description exactly as-is.
6. Sermon placeholder: If an item's description contains a [Preacher Name] placeholder,
   propose replacing only the [Preacher Name] with the volunteer assigned to the "Preacher"
...
function buildAnalysisUserPrompt(slackMessage, pcoPlan, campusName) {
  ...
  return `## Campus
This analysis is for the **${campusName}** campus. The Slack message may contain instructions for multiple campuses. Only act on content that is explicitly directed at "${campusName}" or at all campuses. Ignore content directed at other campuses entirely — do not include it in manual steps or proposed changes.

## Slack Service Plan Message
